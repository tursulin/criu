...
retry_src_va:
	...
	err = amdgpu_va_range_alloc(h_dev, amdgpu_gpu_va_range_general, src_bo_size, 0x1000, 0, &gpu_addr_src,
				    &h_va_src, 0);
	if (err) {
		pr_err("failed to alloc VA for src bo - %s", strerror(-err));
		goto err_src_va;
	}
...
retry_dst_va:
	...
	err = alloc_and_map(h_dev, packets_per_buffer * 28, AMDGPU_GEM_DOMAIN_GTT, &h_bo_ib, &h_va_ib, &gpu_addr_ib,
			    (void **)&ib);
	if (err) {
		pr_err("failed to allocate and map ib/rb - %s", strerror(-err));
...
err_dst_bo_map:
	...
	if (err2)
		pr_err("dest range free failed - %s", strerror(-err2));
err_dst_va:
	if (!do_not_free) {
		err2 = amdgpu_bo_free(h_bo_dst);
		if (err2)
			pr_err("dest bo free failed - %s", strerror(-err2));
	}
err_dst_bo_prep:
...
err_src_bo_map:
	...
	if (err2)
		pr_err("src range free failed - %s", strerror(-err2));
err_src_va:
	err2 = amdgpu_bo_free(h_bo_src);
	if (err2)
		pr_err("src bo free failed - %s", strerror(-err2));
	return err;
}
...
void *dump_bo_contents(void *_thread_data)
{
	...
	if (ret) {
		pr_err("failed to initialize device - %s", strerror(-ret));
		goto exit;
	}
...
static int amdgpu_plugin_restore_drm_file(int id, bool *retry_needed)
{
	...
	if (restore_maps.mapped_cnt) {
		...
		if (!target_gpu_id) {
			pr_perror("Unable to map gpu_id 0x%04x\n", rd->gpu_id);
			fd = -ENODEV;
			goto fail;
		}
	} else {
		...
		if (num_gpus != 1) {
			pr_perror("Unexpectedly found %u GPUs\n", num_gpus);
			fd = -EINVAL;
			goto fail;
		} else if (target_gpu_id != rd->gpu_id) {
			pr_perror("Unexpectedly found gpu_id 0x%04x (expected 0x%04x)\n",
	...
	if (!tp_node) {
		pr_perror("Unable to find target gpu_id=0x%04x\n",
			  target_gpu_id);
		fd = -ENODEV;
		goto fail;
	}
